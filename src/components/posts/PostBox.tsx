...
export default function PostBox({ post }: PostBoxProps) {
	...
	const imgageRef = ref(storage, post?.imageUrl);

	const handleDelete = async () => {
		const confirm = window.confirm("해당 게시글을 삭제 하시겠습니까?");
		if (confirm) {
			if (post?.imageUrl) {
				deleteObject(imgageRef).catch((error) => {
					console.log(error);
				});
			}
		...
		<div className="post__box" key={post?.id}>
			<Link to={`/posts/${post?.id}`}>
				<div className="post__box-profile">
					...
					{post?.imageUrl && (
						<div className="post__image-dev">
							<img src={post?.imageUrl} alt="post img" className="post__iamge" width={100} height={100} />
						</div>
					)}
